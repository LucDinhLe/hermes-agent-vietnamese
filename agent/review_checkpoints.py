...
class ReviewCheckpointController:
    ...
    def __init__(
        ...
        *,
        ...
        record_usage: Callable[[ReviewResult, ReviewRequest], None] | None = None,
    ) -> None:
    ...
    def evaluate(self, request: ReviewRequest) -> ReviewCheckpointDecision:
        ...
        if decision.result is not None and decision.result.usage:
            try:
                if self._record_usage is not None:
                    self._record_usage(decision.result, request)
            except Exception:
                # Accounting is observable evidence, but must never change the
                # answer path after a provider call has already completed.
                pass
        return decision
    ...
    def _decision_for(
        ...
        if result.verdict == "REVISE":
            ...
            return ReviewCheckpointDecision(
                # Advisor is an optional quality layer, not the owner of the
                # conversation. After the bounded rewrite, keep the revised
                # main-model answer instead of turning reviewer uncertainty
                # into a dead-end assistant response.
                action="continue",
                result=result,
    ...
    def _emit_decision(
        ...
        event = {
            ...
            "usage": dict(result.usage) if result is not None and result.usage is not None else None,
...
def review_tool_checkpoint(
    ...
    *,
    ...
    # Reads cannot mutate user state. Reviewing them adds latency and burns a
    # second subscription-model call without improving safety or correctness.
    if actions and all(action["effect"] == "read" for action in actions):
        return ReviewCheckpointDecision(action="continue", reason="read_only_plan")
...
def bounded_review_evidence(messages: list[Any]) -> list[str]:
    """Return recent, bounded tool evidence without resending conversation history."""

    evidence: list[str] = []
    for message in reversed(messages[-80:]):
        ...
        if text:
            evidence.append(f"{name}: {text[:2_000]}")
        if len(evidence) >= 20:
            break
    evidence.reverse()
    return evidence
