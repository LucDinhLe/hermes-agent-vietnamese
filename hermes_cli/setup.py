...
def _apply_default_agent_settings(config: dict):
    ...
    config.setdefault("agent", {})["max_turns"] = 50
    ...
    print_info("  Max iterations: 50 (legacy failsafe; TurnGovernor is primary)")
...
def setup_agent_settings(config: dict):
    ...
    current_max = str(cfg_get(config, "agent", "max_turns", default=50))
    print_info("Maximum tool-calling iterations per conversation.")
    print_info("Higher = more complex tasks, but costs more tokens.")
    print_info(
        f"Press Enter to keep {current_max}. The per-turn governor is primary; "
        "send 'continue' for longer work."
    )
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        max_turns = cfg_get(config, "agent", "max_turns", default=50)
