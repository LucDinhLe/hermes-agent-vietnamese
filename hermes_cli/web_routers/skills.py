...
from hermes_cli.web_models import (
    ...
    SkillsUpdateRequest,
    TaskSkillDiscoveryRequest,
    WorkProfileApplyRequest,
    WorkProfileRecommendRequest,
)
...
def _installed_skill_names() -> set[str]:
    """Return installed names inside the caller's active profile scope."""
    from tools.skills_tool import _find_all_skills
...
@router.get("/api/skills/work-profile")
async def get_work_profile(profile: Optional[str] = None):
    from hermes_cli.capability_profile import work_profile_state

    def _run():
        with _profile_scope(profile):
            installed = _installed_skill_names()
            state = work_profile_state(load_config(), installed_skills=installed)
        return {
...
@router.post("/api/skills/work-profile/recommend")
async def recommend_work_profile(
    body: WorkProfileRecommendRequest,
    profile: Optional[str] = None,
):
    from hermes_cli.capability_profile import recommend_skills
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            result = recommend_skills(
                ...
                installed_skills=_installed_skill_names(),
            )
        return {
...
@router.put("/api/skills/work-profile")
async def apply_work_profile_endpoint(
    body: WorkProfileApplyRequest,
    profile: Optional[str] = None,
):
    from hermes_cli.capability_profile import apply_work_profile, work_profile_state
...
async def discover_task_skill_route(
    body: TaskSkillDiscoveryRequest,
    profile: Optional[str] = None,
):
    from hermes_cli.capability_profile import discover_task_skills, work_profile_state
    from hermes_cli.skills_config import get_disabled_skills
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            allowed = (
                ...
                else installed - get_disabled_skills(config)
            )
