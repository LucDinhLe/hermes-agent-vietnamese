...
1. Gói Windows x64 cuối cùng bị Windows Application Control chặn thực thi trên
   máy thử sau khi hash thay đổi; vì vậy chưa thể hoàn tất lại smoke test trên
   đúng commit cuối.
...
Có thể cân nhắc **thử nội bộ có kiểm soát** sau khi Windows policy cho phép đúng
artifact cuối và GitHub workflow tạo job thật. Không giữ hoặc nhập dữ liệu Hermes
cũ trong bất kỳ bước nào.
...
| Windows artifact cuối | Build đạt, chạy bị chặn | NSIS x64 tạo thành công từ `c646de336`, stamp sạch; payload contract đạt và không chứa `tar`, `image-size`, state DB, `.env` hay credentials. Application Control chặn `Hermes.exe` trước khi renderer khởi động. |
...
- Kích thước: `121419167` byte
- SHA-256: `1140C2E1F654FBEA980B5386B3698264A249F32A9F9A44916B4BA665FE645372`
- Install stamp: `c646de3362e47394ef8ccb672f0eaf5fc7a3341a`
- Trạng thái chữ ký: `NotSigned`
- SHA-256 `win-unpacked/Hermes.exe`:
  `80AC3DD066961B531BC417C50C7CF418C961E9B5173CD11A6D08A1B5C745B682`
...
- Nếu thay đổi đã được push sau này nhưng chưa release, revert lần lượt từ
  `c646de336` về trước; chạy lại lock/security tests sau mỗi revert.
...
1. Cho phép/ký đúng artifact Windows cuối, rồi chạy lại fresh-install smoke trên
   hồ sơ mới và xác minh toàn bộ UI ở commit cuối.
