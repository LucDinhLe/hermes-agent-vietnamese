# Hermes Vietnamese `vi-v0.32.0-3` — kế hoạch hotfix an toàn phiên
...
Candidate này chỉ sửa sự cố Dự án làm người dùng tưởng lịch sử phiên đã mất và
blocker đóng gói phát hiện trong lượt build `vi-v0.32.0-2`:
...
7. payload cài đúng cây dependency đã xuất từ `uv.lock`, bỏ cấu hình override
   và không phân giải/nâng cấp lại trong chế độ bắt buộc hash.
...
- Tag dự kiến: `vi-v0.32.0-3`.
- Version hiển thị: `0.32.0-vi.3`.
...
- Không vá đè tag hoặc asset `vi-v0.32.0-1`; không tái sử dụng candidate `-2`.
...
- Regression payload chứng minh `uv pip` nhận cây export đầy đủ bằng
  `--require-hashes --no-deps --no-config`, không `--upgrade`, trên Windows
  x64.
