...
import { VIETNAMESE_EDITION_LOCALES } from './i18n'
...
function runtimePlatform(): { architecture: string; platform: string } {
  ...
  const architecture = /arm64|aarch64/i.test(userAgent) ? 'arm64' : /x86_64|x64|win64|amd64/i.test(userAgent) ? 'x64' : 'unknown'
  ...
  return (
    <main className="mx-auto flex h-full w-full max-w-3xl flex-col gap-8 overflow-y-auto px-6 py-10 text-(--ui-text-primary)">
      <header className="space-y-2">
        <p className="text-xs font-medium uppercase tracking-wide text-(--ui-text-tertiary)">{EDITION_INFO.productVersion}</p>
