...
test('v32.1 safety update is an explicit unsigned x64 community pilot with its own exact lifecycle lane', () => {
  ...
  assert.doesNotMatch(candidate, /matrix\.id == 'windows-x64' && needs\.verify\.outputs\.tag == 'vi-v0\.32\.1-5'/)
  assert.match(candidate, /build_matrix: \$\{\{ steps\.candidate\.outputs\.build_matrix \}\}/)
  assert.match(candidate, /if \[\[ "\$RELEASE_TAG" == "vi-v0\.32\.1-5" \]\]/)
  assert.match(candidate, /matrix: \$\{\{ fromJSON\(needs\.verify\.outputs\.build_matrix\) \}\}/)
  assert.match(runtimeSmoke, /exact-bytes:[\s\S]*?inputs\.tag != 'vi-v0\.32\.1-5'/)
  assert.match(runtimeSmoke, /evidence:[\s\S]*?inputs\.tag != 'vi-v0\.32\.1-5'/)
  assert.match(runtimeSmoke, /windows_x64_sha256:/)
  assert.match(runtimeSmoke, /v321-windows-lifecycle:/)
  assert.match(v321Lane, /if: inputs\.tag == 'vi-v0\.32\.1-5' && inputs\.release_class == 'community-prerelease'/)
