...
test('v32.1 safety update is an unsigned x64 pilot whose exact bytes are independent of its lifecycle harness', () => {
  ...
  assert.doesNotMatch(v321Lane, /ref: \$\{\{ inputs\.tag \}\}/)
  assert.doesNotMatch(v321Lane, /gh api .*commits/)
  assert.match(v321Lane, /CANDIDATE_COMMIT=\$candidateCommit/)
  assert.match(v321Lane, /HARNESS_COMMIT=\$harnessCommit/)
  ...
  assert.match(v321Promotion, /lifecycle_harness_commit="\$\(jq -r \.head_sha lifecycle-run\.json\)"/)
  assert.doesNotMatch(v321Promotion, /test "\$\(jq -r \.head_sha lifecycle-run\.json\)" = "\$candidate_commit"/)
...
test('pilot promotion stays prerelease, validates every byte, and discloses missing native smoke', () => {
  ...
  assert.match(pilotPromotion, /validationHarnessCommit/)
  assert.match(pilotPromotion, /staging failure is not postcheck-only/)
  assert.match(pilotPromotion, /Tải lại và đối chiếu byte đã staging/)
  assert.match(pilotPromotion, /draft creation\/upload did not complete before postcheck failure/)
