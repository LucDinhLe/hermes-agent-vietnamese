...
const account = {
  ...
  async *streamGenerate(model: string, request: unknown, _signal?: AbortSignal) {
    ...
    yield { candidates: [{ content: { parts: [{ text: 'chào' }] }, finishReason: 'STOP' }], usageMetadata: { promptTokenCount: 3, candidatesTokenCount: 2, totalTokenCount: 5 } }
  }
}
...
function call(path: string, init: RequestInit & { auth?: string } = {}) {
  return fetch(`http://127.0.0.1:${server.port}${path}`, {
    ...init,
    headers: { 'content-type': 'application/json', authorization: `Bearer ${init.auth ?? server.apiKey}`, ...(init.headers ?? {}) }
  })
}
...
test('từ chối khi thiếu/sai khoá; /v1/models liệt kê model Gemini', async () => {
  ...
  assert.deepEqual(body.data.map(m => m.id), GOOGLE_BRIDGE_MODELS)
})

test('chat completions không stream: dịch request, gộp chunk', async () => {
  const res = await call('/v1/chat/completions', {
    method: 'POST',
    body: JSON.stringify({ model: 'gemini-2.5-flash', messages: [{ role: 'system', content: 'S' }, { role: 'user', content: 'hi' }] })
  })

  assert.equal(res.status, 200)
  const body = (await res.json()) as { choices: { message: { content: string }; finish_reason: string }[]; usage: { total_tokens: number }; model: string }
  ...
  assert.deepEqual((lastCall?.request as { systemInstruction: unknown }).systemInstruction, { role: 'user', parts: [{ text: 'S' }] })
})
...
test('chat completions stream: SSE chunk OpenAI, kết thúc bằng [DONE]', async () => {
  ...
  const chunks = events.slice(0, -1).map(e => JSON.parse(e.slice(6)) as { model: string; choices: { delta: { content?: string }; finish_reason: string | null }[]; usage?: unknown })
...
test('lỗi: chưa đăng nhập → 401; JSON hỏng → 400; Code Assist HTTP 429 → 429', async () => {
  ...
  const res = await call('/v1/chat/completions', { method: 'POST', body: '{"messages":[{"role":"user","content":"x"}]}' })
