...
test('toGeminiRequest: system → systemInstruction, user/assistant → contents, gộp lượt cùng vai', () => {
  const g = toGeminiRequest({
    ...
    messages: [
      ...
      { role: 'user', content: [{ type: 'text', text: 'ảnh này' }, { type: 'image_url', image_url: { url: 'data:image/png;base64,QUJD' } }] },
  ...
  assert.deepEqual(g.contents[0].parts, [{ text: 'xin chào' }, { text: 'ảnh này' }, { inlineData: { mimeType: 'image/png', data: 'QUJD' } }])
...
test('toGeminiRequest: tools → functionDeclarations, tool_calls → functionCall, role tool → functionResponse với đúng tên', () => {
  const g = toGeminiRequest({
    ...
    messages: [
      ...
      { role: 'assistant', content: null, tool_calls: [{ id: 'call_1', type: 'function', function: { name: 'weather', arguments: '{"city":"Da Lat"}' } }] },
      { role: 'tool', tool_call_id: 'call_1', content: '{"temp":18}' }
    ],
    tools: [{ type: 'function', function: { name: 'weather', description: 'x', parameters: { type: 'object', additionalProperties: false, properties: { city: { type: 'string', default: '' } } } } }],
    tool_choice: 'required'
  })

  assert.deepEqual(g.tools, [{ functionDeclarations: [{ name: 'weather', description: 'x', parameters: { type: 'object', properties: { city: { type: 'string' } } } }] }])
  assert.deepEqual(g.toolConfig, { functionCallingConfig: { mode: 'ANY' } })
  assert.deepEqual(g.contents[1], { role: 'model', parts: [{ functionCall: { id: 'call_1', name: 'weather', args: { city: 'Da Lat' } } }] })
  assert.deepEqual(g.contents[2], { role: 'user', parts: [{ functionResponse: { id: 'call_1', name: 'weather', response: { temp: 18 } } }] })
})
...
test('toGeminiRequest: lượt đầu là assistant → chèn user giả; tool result không phải JSON → {result}', () => {
  const g = toGeminiRequest({
    model: 'm',
    messages: [
      { role: 'assistant', content: 'a', tool_calls: [{ id: 'c', type: 'function', function: { name: 'f', arguments: 'not json' } }] },
  ...
  assert.deepEqual(g.contents[2].parts[0], { functionResponse: { id: 'c', name: 'f', response: { result: 'plain text' } } })
})

test('sanitizeSchema: bỏ additionalProperties/$schema/default ở mọi tầng', () => {
  assert.deepEqual(
    sanitizeSchema({ $schema: 'x', type: 'object', additionalProperties: false, properties: { a: { type: 'array', items: { type: 'string', default: 'z' } } } }),
...
test('StreamTranslator: text + functionCall + finish → chunk OpenAI với tool_calls index và usage', () => {
  ...
  const a = t.translate({ candidates: [{ content: { parts: [{ text: 'Xin ' }, { thought: true, text: 'suy nghĩ' }] } }] })
  assert.equal(a.length, 1)
  assert.deepEqual((a[0] as { choices: { delta: unknown }[] }).choices[0].delta, { content: 'Xin ' })

  const b = t.translate({
    candidates: [{ content: { parts: [{ functionCall: { name: 'weather', args: { city: 'Da Lat' } } }] }, finishReason: 'STOP' }],
    usageMetadata: { promptTokenCount: 10, candidatesTokenCount: 5, thoughtsTokenCount: 2, totalTokenCount: 17 }
  })

  assert.equal(b.length, 2)
  const delta = (b[0] as { choices: { delta: { tool_calls: { index: number; id: string; function: { name: string; arguments: string } }[] } }[] }).choices[0].delta
  ...
  const fin = b[1] as { choices: { finish_reason: string }[]; usage: { prompt_tokens: number; completion_tokens: number; total_tokens: number } }
...
test('toOpenAiCompletion: gộp chunk thành một completion; MAX_TOKENS → length', () => {
  const c = toOpenAiCompletion('chatcmpl-1', 'm', [
    ...
    { candidates: [{ content: { parts: [{ text: 'hai' }] }, finishReason: 'MAX_TOKENS' }], usageMetadata: { promptTokenCount: 1, candidatesTokenCount: 2, totalTokenCount: 3 } }
