...
function previewMessage(phase: DowngradePhase, fallbackTierName: string, isVi: boolean): null | string {
  if (phase.kind === 'previewing') {
    return isVi ? 'Đang kiểm tra thay đổi…' : 'Checking this change…'
  }
  ...
  switch (preview.effect) {
    case 'blocked':
      return preview.reason ?? (isVi ? 'Không thể thực hiện thay đổi này tại đây.' : 'That change cannot be made here.')

    case 'no_op':
      return isVi
        ? `Bạn đang dùng ${targetName} — không cần thay đổi.`
        : `You are already on ${targetName} — nothing to change.`

    case 'scheduled':
      return isVi
        ? `Chuyển sang ${targetName} — có hiệu lực ${formatBillingDate(preview.effective_at)}. Chưa thu phí ngay; gói hiện tại vẫn dùng đến thời điểm đó.${creditsDelta ? ` Tín dụng hàng tháng thay đổi: ${creditsDelta}.` : ''}`
        : `Change to ${targetName} — takes effect ${formatBillingDate(preview.effective_at)}. No charge now; you keep your current plan until then.${creditsDelta ? ` Monthly credits change: ${creditsDelta}.` : ''}`

    default:
      return isVi ? 'Không thể lên lịch thay đổi này tại đây.' : 'This change cannot be scheduled here.'
  }
}
...
function DowngradeConfirm({ flow, tier }: { flow: DowngradeFlow; tier: BillingPlanTierView }) {
  ...
  const message = previewMessage(phase, tier.name, isVi)
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
            {tier.priceDisplay}/{isVi ? 'tháng' : 'mo'}
          </div>
        </div>
      </div>
      ...
      <div className="mt-auto min-w-0 pt-1">
        {isCurrent && <Pill tone="primary">{isVi ? 'Gói hiện tại' : 'Current plan'}</Pill>}

        {tier.state === 'scheduled' && <Pill>{isVi ? 'Đã lên lịch' : 'Scheduled'}</Pill>}
  ...
  return (
    <div className="@container">
      <div className="mb-2.5 flex items-center gap-2 pt-2 text-[length:var(--conversation-text-font-size)] font-medium">
        <Button
          aria-label={isVi ? 'Quay lại thanh toán' : 'Back to billing'}
      ...
      {tiers.length > 0 ? (
        ...
        <div className="rounded-xl bg-(--ui-bg-quaternary) p-4 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
          {isVi ? 'Hiện chưa có gói nào khác để chuyển sang.' : 'No plans are available to change to right now.'}
        </div>
      )}
    </div>
  )
