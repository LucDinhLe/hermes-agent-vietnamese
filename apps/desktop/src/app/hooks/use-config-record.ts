...
export const hermesConfigKey = (profile?: null | string, connectionId?: null | string) => {
  const source = connectionId?.trim()

  if (!source) {
    return profile == null ? HERMES_CONFIG_KEY : ([...HERMES_CONFIG_KEY, normalizeProfileKey(profile)] as const)
  }

  return [...HERMES_CONFIG_KEY, source, normalizeProfileKey(profile)] as const
}
...
export const useHermesConfigRecord = (profile?: null | string, connectionId?: null | string) =>
  useQuery({
    ...
    queryFn: () => getHermesConfigRecord(profile ?? undefined, connectionId),
    staleTime: 0
  })
...
export const hermesConfigCacheWriter = (profile?: null | string, connectionId?: null | string) =>
  writeCache<HermesConfigRecord>(hermesConfigKey(profile, connectionId))

export const invalidateHermesConfig = (profile?: null | string, connectionId?: null | string) =>
  queryClient.invalidateQueries({ queryKey: hermesConfigKey(profile, connectionId) })
