...
function ClarifyToolSinglePending({ fromArgs, request }: { fromArgs: ClarifyArgs; request: ClarifyRequest | null }) {
  ...
  const respond = useCallback(
    async (answer: string) => {
      ...
      try {
        await requestForGatewayEventSource<{ ok?: boolean }>(matchingRequest.sessionId, 'clarify.respond', {
    ...
    [copy.notReady, copy.sendFailed, matchingRequest, ready]
  )
...
function ClarifyToolBatchPending({ request }: { request: ClarifyRequest | null }) {
  ...
  const confirmAll = useCallback(async () => {
    ...
    try {
      ...
      for (const question of questions) {
        ...
        await requestForGatewayEventSource<{ ok?: boolean }>(request.sessionId, 'clarify.respond', {
          answer: answer ?? '',
  ...
  }, [copy.notReady, copy.sendFailed, questions, request, stagedAnswer])
