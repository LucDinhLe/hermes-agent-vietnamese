...
export const vi = defineLocale({
  common: {
    ...
    truncatedCharacters: count =>
      `${count.toLocaleString('vi-VN')} ký tự khác đã được lược bớt — dùng Sao chép để lấy toàn bộ nội dung.`,
    sessionUsageSummary: (calls, input, output, total) =>
      `Mức sử dụng: ${calls.toLocaleString('vi-VN')} lượt gọi · ${input.toLocaleString('vi-VN')} đầu vào / ${output.toLocaleString('vi-VN')} đầu ra · ${total.toLocaleString('vi-VN')} tổng cộng`,
  ...
  keybinds: {
    ...
    actions: {
      ...
      'view.selectionToComposer': 'Đưa nội dung đã chọn vào khung soạn thảo',
  ...
  settings: {
    ...
    search: {
      placeholder: 'Tìm trong toàn bộ cài đặt…',
      pill: 'Tìm kiếm'
    },
    mcp: {
    ...
    model: {
      ...
      moa: {
        title: 'Hội đồng cố vấn (MoA)',
        description:
          'Thiết lập các cấu hình Mixture of Agents. Model tham chiếu đóng góp ý kiến; model tổng hợp tạo câu trả lời cuối cùng.',
        preset: 'Cấu hình',
        enabled: 'Đang bật',
        setDefault: 'Đặt làm mặc định',
        delete: 'Xóa',
        newPreset: 'tên cấu hình mới',
        addPreset: 'Thêm cấu hình',
        defaultLabel: 'Mặc định',
        disableReference: index => `Tắt model tham chiếu ${index}`,
        enableReference: index => `Bật model tham chiếu ${index}`,
        remove: 'Gỡ',
        reference: index => `Model tham chiếu ${index}`,
        addReference: 'Thêm model tham chiếu',
        aggregator: 'Model tổng hợp'
      },
      tasks: {
  ...
  commandCenter: {
    ...
    mcpServers: 'MCP',
    archivedChats: 'Cuộc trò chuyện được lưu trữ',
    sections: { maintenance: 'Bảo trì', sessions: 'Phiên', system: 'Hệ thống', usage: 'Thống kê sử dụng' },
    sectionDescriptions: {
  ...
  profiles: {
    ...
    switchConnectionFailed: name => `Không thể kết nối tới ${name}`,
...
Để trống để giữ ${mode} mặc định.`,
  ...
  assistant: {
    ...
    tool: {
      ...
      toolPayload: 'Dữ liệu công cụ',
      arguments: 'Tham số',
      result: 'Kết quả',
      searchResults: 'Kết quả tìm kiếm',
      search: 'Tìm kiếm',
      standardOutput: 'Đầu ra chuẩn',
