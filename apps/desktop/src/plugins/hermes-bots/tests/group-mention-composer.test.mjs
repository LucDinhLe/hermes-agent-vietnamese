...
test('group room composers mount GroupMentionInput, not bare Input', () => {
  ...
  const replyComposer = source.match(/jsx\(GroupMentionInput, \{\s*'aria-label': 'Trả lời trong luồng'/)
  ...
  assert.ok(!/jsx\(Input, \{\s*'aria-label': 'Trả lời trong luồng'/.test(source))
