...
test('source contract: bot rows and Active now activate the owner before canonical chat', () => {
  ...
  assert.match(pluginSource, /const open = async \(\) => \{[\s\S]*await prepareBotSource\(bot, pinnedChat\)[\s\S]*openBotCanonicalChat\(bot\.name, pinnedChat, bot\.last_session, openOwner\)/)
  assert.match(pluginSource, /onOpen: bot => \{[\s\S]*await prepareBotSource\(bot, pinnedChat\)[\s\S]*openBotCanonicalChat\(bot\.name, pinnedChat, bot\.last_session, openOwner\)/)
  assert.match(pluginSource, /openBotSessionsWorkspace\(bot\)[\s\S]*children: copy\('common\.sessions'\)/)
})
...
test('source contract: workspaces disclose the bounded recent-session inventory', () => {
  ...
  assert.match(pluginSource, /copy\('sessions\.recent', PROFILE_SESSION_LIST_LIMIT\)/)
  assert.match(pluginSource, /copy\('sessions\.noRecentMatch', PROFILE_SESSION_LIST_LIMIT\)/)
...
test('source contract: session controls expose filter and selection state to assistive technology', () => {
  assert.match(pluginSource, /'aria-label': copy\('sessions\.filterAria'\)/)
  assert.match(pluginSource, /placeholder: copy\('sessions\.filterPlaceholder'\)/)
