...
import { expectedBundledProvenanceFromEnv, readAndValidateBundledProvenance } from '../scripts/packaged-provenance.mjs'
import { packagedAppDirectoryName } from '../scripts/packaged-layout.mjs'
...
function resolvePackagedBinaryPath(): string {
  ...
  const packagedDirectory = packagedAppDirectoryName(process.platform, process.arch)
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, packagedDirectory, 'Hermes.exe')
  }

  if (process.platform === 'darwin') {
    return path.join(RELEASE_ROOT, packagedDirectory, 'Hermes.app', 'Contents', 'MacOS', 'Hermes')
  }

  return path.join(RELEASE_ROOT, packagedDirectory, 'Hermes')
}
...
function buildPackagedAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
 ...
 */
export async function setupPackagedMockBackend(options: MockBackendOptions = {}): Promise<PackagedMockBackendFixture> {
  if (!packagedBinaryExists()) {
    throw new Error(`Built app binary not found: ${PACKAGED_BINARY_PATH}. Build the candidate first.`)
  }
