...
test("current Vietnamese candidate keeps product, technical, and upstream versions separate", () => {
  ...
  const expectedTag = `vi-v${VI_PRODUCT_RELEASE.technicalVersion}-4`
  ...
  assert.equal(candidate.iteration, 4)
  assert.equal(candidate.appVersion, `${VI_PRODUCT_RELEASE.technicalVersion}-vi.4`)
...
test("candidate checkout binds a clean HEAD to the exact local tag", () => {
  ...
  assert.throws(
    () => validateVietnameseCandidateCheckout({ ...input, tagCommit: "b".repeat(40) }),
    /points to/,
  )
  assert.throws(
    () => validateVietnameseCandidateCheckout({ ...input, status: " M README.md" }),
    /must be clean/,
  )
})
...
test("prerelease promotion requires the exact prepared public candidate", () => {
  ...
  assert.deepEqual(
    validateFeaturedCandidatePromotion({ featuredCandidate, tag: featuredCandidate.tag }),
    { published: true, tag: featuredCandidate.tag },
  )
  assert.throws(
...
test("release notes cannot claim a different publication class", () => {
  const communityNotes =
    "Lớp phát hành: community prerelease, chưa phải stable. Latest hiện vẫn là bản cũ."
  ...
  assert.deepEqual(
    validateVietnameseReleaseNotesForClass({ body: stableNotes, releaseClass: "stable" }),
    { classMatches: true, releaseClass: "stable" },
  )
  assert.throws(
    () =>
      validateVietnameseReleaseNotesForClass({
