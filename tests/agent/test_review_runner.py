...
class _RecordingCompletion:
    ...
    def __call__(
        ...
        *,
        ...
        self.calls.append(_CompletionCall(
...
def _complete_ok(**payload_overrides: object) -> _RecordingCompletion:
...
def test_subscription_route_runs_once_without_tools_and_attests_actual_route():
    ...
    assert "same language as the user's objective" in system_prompt
    assert "direct, user-facing clarification question" in system_prompt
