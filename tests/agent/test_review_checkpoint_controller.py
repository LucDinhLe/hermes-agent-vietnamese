...
class _Runner:
    ...
    def __call__(self, request: ReviewRequest) -> ReviewResult:
        ...
        return replace(self.result, checkpoint_id=request.checkpoint_id)


def _runner(result: ReviewResult) -> _Runner:
...
def test_revision_count_is_bounded_and_then_preserves_main_answer():
    ...
    assert exhausted.action == "continue"
...
def test_usage_is_emitted_and_recorded_once_for_a_billed_checkpoint():
    events = []
    recorded = []
    controller = ReviewCheckpointController(
        session_id="session-1",
        config=ReviewCheckpointConfig(enabled=True),
        run_review=_runner(_result("PASS", usage={"input_tokens": 31, "output_tokens": 7})),
        emit=events.append,
        record_usage=lambda result, request: recorded.append((result.usage, request.phase)),
    )
    ...
    assert replay is first
    assert events[-1]["usage"] == {"input_tokens": 31, "output_tokens": 7}
    assert recorded == [({"input_tokens": 31, "output_tokens": 7}, "plan")]
