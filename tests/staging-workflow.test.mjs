...
test('V33 dev.10 version is one immutable edition fact', () => {
  assert.equal(shellPackage.version, '0.33.0-dev.10')
...
test('Windows staging builds once and tests exact transition bytes', () => {
  ...
  assert.match(workflow, /Hermes-0\.33\.0-dev\.10-win-x64\.exe/)
  assert.doesNotMatch(workflow, /resident-runtime|agent-payload|stage-resident-runtime|verify-resident-payload/i)
  ...
  assert.match(workflow, /-CandidateVersion '0\.33\.0-dev\.10'/)
