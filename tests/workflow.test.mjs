...
test('Windows validation runs the source and build gates without publishing', () => {
  ...
  assert.equal(workflow.match(/npm run test:desktop:platforms --workspace apps\/desktop/g)?.length, 2)
  ...
  assert.match(workflow, /steps\.electron_upstream\.outputs\.exit_code/)
