...
test('Windows validation runs the source and build gates without publishing', () => {
  ...
  assert.match(workflow, /working-directory: \.work\/upstream\s+shell: pwsh\s+run: npm ci/)
  assert.equal(workflow.match(/npm run test:desktop:platforms --workspace apps\/desktop/g)?.length, 3)
  assert.equal(workflow.match(/continue-on-error: true/g)?.length, 3)
  assert.match(workflow, /node scripts\/compare-electron-baseline\.mjs/)
  assert.match(workflow, /steps\.electron_candidate\.outputs\.exit_code/)
  assert.match(workflow, /steps\.electron_upstream_before\.outputs\.exit_code/)
  assert.match(workflow, /steps\.electron_upstream_after\.outputs\.exit_code/)
  assert.match(workflow, /working-directory: \.work\/upstream\s+shell: pwsh\s+run: git diff --exit-code/)
  assert.match(workflow, /electron-baseline-comparison\.json/)
